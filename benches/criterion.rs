...
fn prepare_zebra(reverse: bool) -> NamedUniverse {
    ...
    if reverse {
        puzzle_terms.reverse();
    }

    u.rule("puzzle($0)", &puzzle_terms).unwrap();
    u
}
...
fn prepare_arithmetic() -> NamedUniverse {
    ...
    u.rule("mul($0,s($1),$2)", &["mul($0,$1,$3)", "add($0,$3,$2)"])
        .unwrap();
